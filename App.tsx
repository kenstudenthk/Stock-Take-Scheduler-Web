...
import { SyncOutlined, WarningFilled, KeyOutlined } from "@ant-design/icons";
        ...
        <div className="flex justify-between w-[130px] px-4 absolute bottom-0">
          <svg width="24" height="24">
            <circle
              ...
              strokeWidth="2"
            />
            <circle cx="12" cy="12" r="5" fill="#DFDFDF" />
          </svg>
          <svg width="24" height="24">
            <circle
              cx="12"
...
function App() {
  ...
  const [graphToken, setGraphToken] = useState<string>(
    localStorage.getItem(TOKEN_CONFIG.storageKeys.graphToken) || "",
  );
  ...
  const sharePointService = useMemo(
    () => new SharePointService(graphToken),
    [graphToken],
  );
  ...
  return (
    <ConfigProvider
      theme={{ token: { colorPrimary: "#05043e", borderRadius: 8 } }}
    >
      <Layout
        onLogout={handleLogout}
        user={currentUser}
          ...
          <>
            <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100 mobile-header-stack">
              <div className="flex items-center gap-4">
                {!isInitialLoading && hasTokenError && !loading && (
                  <TruckFlagNotice />
                )}
                <Button
                ...
                {currentUser ? (
                  <Space>
                    <div className="text-right hidden sm:block">
                      <div className="text-[12px] font-bold text-[#05043e]">
                        {currentUser.Name}
                      </div>
