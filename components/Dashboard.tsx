...
export const Dashboard: React.FC<{
  ...
  const stats = useMemo(() => {
    ...
    const completed = activeShops.filter(
      (s) => s.status === "Done" || s.status === "Re-Open",
    ).length;
  ...
  const checkDateAvailability = (date: dayjs.Dayjs, shop: Shop) => {
    ...
    if (isHoliday(dateStr)) return { valid: false, reason: "Holiday" };
    ...
    if (shopsOnDay.length > 0) {
      ...
      if (shop.is_mtr !== isDayMtrOnly) {
        return {
          valid: false,
          reason: isDayMtrOnly ? "MTR Day Only" : "Street Shops Only",
        };
      }
  ...
  const handleConfirmReschedule = async () => {
    ...
    const shopsOnNewDay = activeShops.filter(
      (s) => s.scheduledDate === formattedDate,
    );
    const newGroupId =
      shopsOnNewDay.length > 0 ? shopsOnNewDay[0].groupId : targetShop.groupId;
    ...
    confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.error("Move to pool failed");
        }
      },
    });
  };
    ...
    <div className="flex flex-col gap-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0">
        <div>
          <Title
            level={2}
            className="m-0 text-slate-800 dark:text-slate-100"
      ...
      >
        ...
        <div className="p-8 flex items-center justify-between border-b border-slate-50 dashboard-header-stack">
          ...
          <div className="flex flex-col gap-2">
            <Text
              strong
              className="text-[10px] text-slate-400 uppercase tracking-widest ml-1"
            >
              Schedule Date
            </Text>
            <DatePicker
                ...
                >
                  ...
                  <div className="flex items-center gap-4 w-full md:flex-1">
                    <img
                      src={shop.brandIcon}
                      alt={shop.brand}
                      className="h-10 w-10 object-contain rounded-lg border border-slate-100 p-1 bg-white"
                    />
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <Text className="text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap">
                          {shop.brand}
                        </Text>
                        {shop.is_mtr && (
                          <Tag
                            color="purple"
                            className="m-0 text-[9px] font-bold border-none px-1"
                          >
                            MTR
                          </Tag>
                        )}
                  ...
                  <div className="w-full md:w-[300px]">
                    <div className="flex items-start gap-1.5 opacity-60">
                      ...
                      <Text className="text-xs text-slate-500 italic leading-snug break-words line-clamp-2 md:line-clamp-1">
                        {shop.address}
                      </Text>
                    </div>
                  </div>
                  ...
                  <div className="w-full md:w-[120px] flex md:justify-center">
                    <Tag
                      className={`m-0 border-none font-black text-[10px] px-3 py-0.5 rounded-md w-fit tag-group-${shop.groupId}`}
                    >
                    ...
                    {isClosed ? (
                      ...
                      hasPermission(currentUser, "close_shop") && (
                        <button
                          ...
                          onClick={() => handleResumeShop(shop)}
                        >
                          <svg
                            className="resume-svgIcon"
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <path
                              d="M3 12a9 9 0 1 1 2.636 6.364M3 12V18M3 12H9"
                              stroke="#16A34A"
                              strokeWidth="2"
                              strokeLinecap="round"
                      ...
                      <>
                        ...
                        {hasPermission(currentUser, "reschedule_shop") && (
                          <button
                            ...
                            onClick={() => handleMoveToPool(shop)}
                          >
                            <svg className="pool-svgIcon" viewBox="0 0 24 24">
                              <path
                                d="M20 7H4a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V8a1 1 0 0 0-1-1zM4 5h16M6 19h12"
                                stroke="#D97706"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            icon={<SyncOutlined />}
            onClick={() => {
              handleMoveToPool(targetShop!);
              setIsReschedOpen(false);
            }}
          >
            Move to Reschedule Pool
          </Button>,
          <Button key="cancel" onClick={() => setIsReschedOpen(false)}>
            Cancel
          </Button>,
          <Button
            key="ok"
      ...
      >
        <div className="py-4">
          <div className="mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <Text
              type="secondary"
              className="text-xs uppercase font-bold block mb-1"
            >
              Target Shop
            </Text>
            <Text strong className="text-lg text-indigo-900">
              {targetShop?.name}
            </Text>
          </div>
          <DatePicker
            ...
            disabledDate={(d) =>
              ...
              !checkDateAvailability(d, targetShop!).valid
            }
            value={reschedDate}
          ...
          {fastestDate && (
            <div className="mt-4 p-3 bg-green-50 rounded-xl border border-green-100 flex items-center justify-between">
              <div>
                <Text className="text-green-700 text-xs block">
                  Fastest Suggest Reschedule Date
                </Text>
                <Text className="text-green-800 font-bold">
                  {fastestDate.format("YYYY-MM-DD (dddd)")}
                </Text>
              </div>
              <Button
                type="link"
                size="small"
                onClick={() => setReschedDate(fastestDate)}
              >
                Use This
              </Button>
            </div>
