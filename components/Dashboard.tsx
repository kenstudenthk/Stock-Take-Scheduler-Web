...
// D. 區域限制：該日已有的店必須與目標店同區域
    ...
    try {
      ...
      if (res.ok) {
        Modal.success({
          ...
          content: `${targetShop.name} moved to ${formattedDate}. Status: Rescheduled.`,
          onOk: () => { setIsReschedOpen(false); onRefresh(); }
        });
      }
  ...
  confirm({
    ...
    content: `Are you sure you want to mark "${shop.name}" as CLOSED? This will update the Schedule Status to Closed in SharePoint.`,
    okText: 'Yes, Close it',
    okType: 'danger',
    cancelText: 'Cancel',
    onOk: async () => {
      try {
        const res = await fetch(
          `https://graph.microsoft.com/v1.0/sites/pccw0.sharepoint.com:/sites/BonniesTeam:/lists/ce3a752e-7609-4468-81f8-8babaf503ad8/items/${shop.sharePointItemId}/fields`,
          {
            ...
            body: JSON.stringify({
              [SP_FIELDS.STATUS]: 'Closed' // 更新 ScheduleStatus 欄位
            })
          }
        );
        if (res.ok) {
          message.success(`"${shop.name}" has been marked as CLOSED.`);
          onRefresh(); // 刷新儀表板數據
        } else {
