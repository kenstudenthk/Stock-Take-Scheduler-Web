...
export const Inventory = ({ token, shops }: Props) => {
  ...
  const handleShopSelect = (value: string) => {
    ...
    if (selectedShop) {
      form.setFieldsValue({
        ...
        cmdb: selectedShop.id, // 自動填入 CMDB 前 5 碼
      });
    }
  };
  ...
  const filteredData = useMemo(() => {
    return data.filter(
      (item) =>
        ...
        (item.assetItemId || "")
          .toLowerCase()
          .includes(filters.assetItemId.toLowerCase()),
    );
  }, [data, filters]);
  ...
  const columns = [
      ...
      render: (record: any) => (
        <Space direction="vertical" size={0}>
          <Text strong className="text-teal-700">
            {record.productTypeEng}
          </Text>
          <Text style={{ color: "#94a3b8", fontSize: "11px" }}>
            {record.productTypeChi}
          </Text>
        </Space>
      ),
    },
      ...
      render: (record: any) => (
        <Space direction="vertical" size={0}>
          ...
          <Text type="secondary" style={{ fontSize: "11px" }}>
            SN: {record.serialNo} | CMDB: {record.cmdb}
          </Text>
        </Space>
      ),
    },
    {
      ...
      render: (record: any) => {
        let color =
          record.stockTakeStatus === "Verified"
            ? "green"
            : record.stockTakeStatus === "New Record"
              ? "orange"
              : "#94a3b8";
        return (
          <Space direction="vertical" size={0}>
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <Space>
          ...
          <Button
            ...
            loading={loading}
            size="large"
            className="bg-teal-600 rounded-xl font-bold shadow-lg"
          >
            Refresh Data
          </Button>
        </Space>
      ...
      <Card className="rounded-2xl border-none shadow-sm">
        <Row gutter={[12, 12]}>
          <Col span={4}>
            <Text
              strong
              className="text-slate-400 text-[10px] uppercase mb-1 block"
            >
              Shop
            </Text>
            <Input
          ...
          <Col span={4}>
            ...
            <Input
              placeholder="SN..."
              onChange={(e) =>
                setFilters({ ...filters, serialNo: e.target.value })
              }
              className="rounded-lg h-10"
            />
          </Col>
          <Col span={8}>
            ...
            <Input
              ...
              onChange={(e) =>
                setFilters({ ...filters, assetItemId: e.target.value })
              }
              className="rounded-lg h-10"
            />
          </Col>
      ...
      <Modal
        title={
          <Space>
            <DatabaseOutlined className="text-teal-600" /> Asset Inventory
            Detail
          </Space>
        }
        open={detailVisible}
        ...
        footer={[
          ...
          <Button
            ...
            className="bg-slate-800 border-none rounded-lg px-8"
          >
            Close
          </Button>,
        ]}
          ...
          <div className="max-h-[70vh] overflow-y-auto px-2">
            <div className="flex justify-between items-start mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              ...
              <div className="text-right">
                <Tag
                  color={
                    selectedItem.stockTakeStatus === "Verified"
                      ? "green"
                      : selectedItem.stockTakeStatus === "New Record"
                        ? "orange"
                        : "default"
                  }
                  className="m-0 px-4 py-1 rounded-lg font-black text-[11px] uppercase"
                >
                  {selectedItem.stockTakeStatus}
                </Tag>
                <div className="mt-2 text-right">
                  <Text strong className="text-teal-600">
                    {selectedItem.inUseStatus}
                  </Text>
                </div>
              </div>
            </div>
            ...
            <Row gutter={24}>
              <Col span={15}>
                <Descriptions
                  ...
                  className="mb-6"
                >
                  <Descriptions.Item label="Shop Name" span={2}>
                    {selectedItem.shopName}
                  </Descriptions.Item>
                  <Descriptions.Item label="Shop Code">
                    {selectedItem.shopCode}
                  </Descriptions.Item>
                  <Descriptions.Item label="Brand">
                    {selectedItem.shopBrand}
                  </Descriptions.Item>
                  <Descriptions.Item label="BU">
                    {selectedItem.businessUnit}
                  </Descriptions.Item>
                  <Descriptions.Item label="Category">
                    <Text strong>{selectedItem.productTypeEng}</Text>
                    <br />
                    <Text className="text-[11px] text-slate-400">
                      {selectedItem.productTypeChi}
                    </Text>
                  </Descriptions.Item>
                </Descriptions>
                <Descriptions
                  ...
                  size="small"
                >
                  <Descriptions.Item label="CMDB No">
                  ...
                  <Descriptions.Item label="IP Address" span={2}>
                    <Tag color="cyan">{selectedItem.ipAddress || "N/A"}</Tag>
                  </Descriptions.Item>
                </Descriptions>
              </Col>
              <Col span={9}>
                <Text
                  strong
                  className="text-slate-500 block mb-2 uppercase text-[11px]"
                >
                ...
                <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <Text
                    strong
                    className="text-[10px] uppercase block mb-3 text-slate-400"
                  >
                    <HistoryOutlined /> Audit Trace
                  </Text>
      ...
      <Modal
        title={
          <Space>
            ...
            {formMode === "new" ? "Add New Asset" : "Edit Asset Detail"}
          </Space>
        }
        open={formVisible}
        ...
        >
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item
                name="shopName"
                label="Shop Name (Type Code or Name to search)"
                rules={[{ required: true }]}
              >
                <AutoComplete
            ...
            <Col span={8}>
              <Form.Item name="shopBrand" label="Shop Brand">
                <Input disabled className="bg-slate-50" />
              </Form.Item>
            </Col>
            <Col span={8}>
            ...
            <Col span={24}>
              <Form.Item
                ...
                rules={[{ required: true }]}
              >
                <Input placeholder="e.g. iPad Pro 11-inch" />
              </Form.Item>
            </Col>
            ...
            <Col span={12}>
              <Form.Item name="assetItemId" label="Asset Item ID">
                <Input placeholder="e.g. EQ-12345" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="brand" label="Manufacturer / Brand">
                <Input placeholder="e.g. Apple" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="serialNo" label="Serial Number">
                <Input placeholder="Unique SN" />
              </Form.Item>
            </Col>
