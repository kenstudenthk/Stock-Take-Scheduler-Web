...
import {
  ...
  InputNumber, Table, Tag, message, Modal, DatePicker, Progress, Steps
} from 'antd';
import {
  ...
  SettingOutlined, EyeOutlined, CloudUploadOutlined,
  ArrowLeftOutlined, ArrowRightOutlined, CloseOutlined,
  FilterOutlined, ScheduleOutlined, ThunderboltOutlined
...
const WizardProgressBar: React.FC<{
  ...
  const getStatusColor = () => {
    switch (status) {
      case 'generating': return DESIGN_COLORS.step2;
      case 'syncing': return DESIGN_COLORS.primary;
      case 'complete': return DESIGN_COLORS.step3;
      case 'error': return DESIGN_COLORS.step1;
      default: return DESIGN_COLORS.primary;
    }
  };

  const getStatusText = () => {
    switch (status) {
      case 'generating': return 'Generating schedule...';
      case 'syncing': return progress ? `Syncing ${progress.current}/${progress.total}...` : 'Syncing...';
      case 'complete': return 'Complete!';
      case 'error': return 'Error occurred';
      default: return `Step ${currentStep + 1} of ${WIZARD_STEPS.length}`;
    }
  };
  ...
  return (
    <div className="wizard-progress-container">
      ...
      <div className="flex items-center justify-between mb-4 relative">
        ...
        <div className="absolute top-5 left-0 right-0 h-1 bg-gray-200 rounded-full" style={{ margin: '0 60px' }} />
        ...
        {WIZARD_STEPS.map((step, index) => {
          ...
          const stepColor = isCompleted ? DESIGN_COLORS.step3 : isActive ? step.color : '#E5E7EB';
          ...
          return (
            ...
            >
              <div
                ...
                `}
                style={{
                  backgroundColor: isCompleted || isActive ? stepColor : undefined,
...
const SyncLoader: React.FC<{ progress?: { current: number; total: number } | null }> = ({ progress }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(13, 148, 136, 0.95)' }}>
    <div className="text-center">
      <div className="relative w-24 h-24 mx-auto mb-6">
        ...
        />
        <CloudUploadOutlined className="absolute inset-0 flex items-center justify-center text-4xl text-white" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} />
      </div>
...
export const SchedulingWizard: React.FC<SchedulingWizardProps> = ({
  ...
  const [wizardStatus, setWizardStatus] = useState<'idle' | 'generating' | 'syncing' | 'complete' | 'error'>('idle');
          ...
          <Row gutter={[24, 24]}>
            ...
            <Col xs={24} lg={9}>
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 h-full">
                <div className="flex items-center gap-2 mb-6">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${DESIGN_COLORS.step1}20` }}>
                    <FilterOutlined style={{ color: DESIGN_COLORS.step1 }} />
                  </div>
                  <Text strong className="text-sm text-slate-600 uppercase tracking-wide">Unplanned Pool</Text>
                </div>
                  ...
                  {regionRemainStats.map(reg => (
                    <div
                      ...
                      onClick={() => {
                        if (selectedRegions.includes(reg.key)) {
                          setSelectedRegions(selectedRegions.filter(r => r !== reg.key));
                        } else {
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs font-bold text-slate-500 uppercase">{reg.displayName}</span>
                      </div>
            ...
            <Col xs={24} lg={15}>
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 h-full">
                <div className="flex items-center gap-2 mb-6">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${DESIGN_COLORS.step2}20` }}>
                    <ControlOutlined style={{ color: DESIGN_COLORS.step2 }} />
                  </div>
                  <Text strong className="text-sm text-slate-600 uppercase tracking-wide">Generation Settings</Text>
                </div>
                ...
                <Row gutter={[16, 20]}>
                  ...
                  <Col span={12}>
                    <Text className="text-slate-400 block mb-2 uppercase text-[10px] font-bold ml-1">Filter Districts</Text>
                    <Select
                      mode="multiple"
                      className="w-full"
                  ...
                  <Col span={12}>
                    <Text className="text-slate-400 block mb-2 uppercase text-[10px] font-bold ml-1">Start Date</Text>
                    <DatePicker
                  ...
                  <Col span={12}>
                    <Text className="text-slate-400 block mb-2 uppercase text-[10px] font-bold ml-1">Shops Per Day</Text>
                    <InputNumber
                      value={shopsPerDay}
                  ...
                  <Col span={12}>
                    <Text className="text-slate-400 block mb-2 uppercase text-[10px] font-bold ml-1">Groups Per Day</Text>
                    <InputNumber
                      value={groupsPerDay}
                ...
                <div className="flex justify-end mt-8">
                  <Button
                    ...
                    disabled={filteredPoolCount === 0 || wizardStatus === 'generating'}
          ...
          {previewStats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                ...
                <div className="text-2xl font-black text-teal-600">{previewStats.totalShops}</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="text-xs text-slate-400 uppercase font-bold mb-1">Working Days</div>
                <div className="text-2xl font-black text-orange-500">{previewStats.totalDays}</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
              ...
              columns={[
                {
                  ...
                  key: 'date',
                  render: d => <b className="text-slate-700">{dayjs(d).format('YYYY-MM-DD (ddd)')}</b>
                },
                {
                  title: 'Group',
                  dataIndex: 'groupId',
          ...
          {wizardStatus === 'complete' ? (
            <div className="max-w-md mx-auto">
              ...
              <Text className="text-slate-500 block mb-6">
                Successfully scheduled {lastBatchResult?.successCount || generatedResult.length} shops to SharePoint.
              </Text>
              <Button
                type="primary"
                size="large"
