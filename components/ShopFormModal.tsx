...
export const ShopFormModal: React.FC<Props> = ({ visible, shop, onCancel, onSuccess, graphToken, shops }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.code) return message.warning("Name/Code required!");
    const isEdit = !!shop;
    ...
    try {
      ...
      if (res.ok) { message.success("Saved!"); onSuccess(); }
  ...
  return (
    <>
      <Modal open={visible} onCancel={onCancel} footer={null} width={900} centered bodyStyle={{ padding: '32px', backgroundColor: '#f8fafc' }}>
        <div className="mb-6"><Title level={3} style={{ margin: 0 }}>{shop ? 'Store Profile Manager' : 'New Store Registration'}</Title><Text type="secondary">Direct SharePoint master data sync.</Text></div>
        ...
        <div className="st-form-section mt-4">
          <div className="flex items-center gap-2 mb-6 text-teal-600 font-bold border-b border-teal-100 pb-2"><PhoneOutlined /> GEOLOCATION</div>
          <Row gutter={[20, 32]}>
            {renderInput("Latitude", "lat", 8)}
            {renderInput("Longitude", "lng", 8)}
            <Col span={8}><Button type="primary" block icon={<SearchOutlined />} onClick={() => setSearchModalVisible(true)} style={{ height: '50px', borderRadius: '10px', background: '#0d9488', fontWeight: 'bold' }}>Location Search</Button></Col>
          </Row>
        </div>
