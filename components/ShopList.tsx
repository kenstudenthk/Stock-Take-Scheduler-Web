...
import { Table, Input, Card, Typography, Space, Tag, DatePicker, message, Modal, Select } from 'antd';
import { 
  ...
  const columns = [
      ...
      render: (record: Shop) => (
        <Space align="center" size={12}>
          ...
          <Space direction="vertical" size={0}>
            <Text strong className={record.status?.toLowerCase() === 'closed' ? 'line-through opacity-50' : ''} style={{ fontSize: '14px' }}>
              {record.name}
            </Text>
            <Space>
              <Tag color={record.status?.toLowerCase() === 'closed' ? 'default' : 'blue'}>
                {record.brand}
              </Tag>
      ...
      render: (record: Shop) => (
        <Space direction="vertical" size={0} className={record.status?.toLowerCase() === 'closed' ? 'opacity-40' : ''}>
          <Text strong style={{ fontSize: '12px' }}>
            <EnvironmentOutlined className="text-teal-600" /> {record.region}
          </Text>
      ...
      render: (record: Shop) => (
        <Space direction="vertical" size={4} style={{ width: '100%' }} onClick={(e) => e.stopPropagation()}>
          <Select 
            ...
            size="small"
            className="w-full"
            value={record.callStatus || undefined} // 改為 value 確保資料同步
          ...
          <Input 
            ...
            defaultValue={record.callRemark} // Remark 使用 defaultValue 配合 onBlur 以減少重複渲染
            onPressEnter={(e: any) => handleCallUpdate(record, 'remark', e.target.value)}
            onBlur={(e: any) => handleCallUpdate(record, 'remark', e.target.value)}
            disabled={record.status?.toLowerCase() === 'closed'}
            style={{ fontSize: '11px' }}
          />
          {record.callDate && (
    ...
    },
    {
      ...
      render: (status: string) => {
        ...
        if (s === 'in-progress') color = 'orange';
        if (s === 'reschedule') color = 'purple';
        if (s === 're-open') color = 'cyan';
      ...
      render: (_: any, record: Shop) => (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', alignItems: 'center' }}>
          {selectedRowId === record.id && (
            <>
              <button 
                className="Btn close-btn-styled" 
                disabled={record.status?.toLowerCase() === 'closed'} 
