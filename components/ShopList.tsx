...
export const ShopList: React.FC<{ shops: Shop[], graphToken: string, onRefresh: () => void }> = ({ shops, graphToken, onRefresh }) => {
  ...
  const columns = [
    ...
    {
      title: 'Contact Information', // ✅ 5. 新增聯絡資訊欄位
      key: 'contact',
      width: '20%',
      render: (record: Shop) => (
        <Space direction="vertical" size={0}>
          <Text style={{ fontSize: '13px' }}><PhoneOutlined className="text-slate-400" /> {record.phone || '--'}</Text>
          <Text type="secondary" style={{ fontSize: '12px' }}><UserOutlined /> {record.contactName || 'N/A'}</Text>
        </Space>
      )
    },
    {
          ...
          <div className="flex justify-between items-center gap-4">
            <Space size="middle">
              ...
              <div className="flex flex-col">
                <Text className="text-[10px] font-bold text-slate-400 uppercase mb-1">Schedule Date Filter</Text>
                <DatePicker 
            ...
            <Input 
              ...
              className="w-80 h-11 rounded-xl bg-slate-50 border-none" 
