...
export const ShopList: React.FC<{
    ...
    const getAvailableValues = (key: keyof Shop, filterKey: string) => {
      const filteredForThisField = shops.filter((s) => {
        return Object.entries(filters).every(([k, v]) => {
          ...
          if (k === "status")
            return s.status?.toLowerCase() === v.toLowerCase();
          return (s as any)[k] === v;
        });
      });
      const uniqueValues = Array.from(
        new Set(
          filteredForThisField
            .map(
              (s) =>
                (s as any)[(key as string) === "bu" ? "businessUnit" : key],
            )
            .filter(Boolean),
        ),
      ).sort();
  ...
  const filteredData = useMemo(() => {
    return shops.filter((s) => {
      ...
      if (dateRange[0] || dateRange[1]) {
        ...
        if (dateRange[0] && dateRange[1]) {
          matchDate =
            scheduledDate.isSameOrAfter(dayjs(dateRange[0]), "day") &&
      ...
      const matchBU =
        filters.bu === "All" || (s as any).businessUnit === filters.bu;
      const matchCall =
        filters.callStatus === "All" ||
        (s as any).callStatus === filters.callStatus;
      const matchMTR =
        filters.mtr === "All" || (filters.mtr === "Yes" ? s.is_mtr : !s.is_mtr);
      const matchGroup =
        filters.group === "All" || s.groupId?.toString() === filters.group;
      const matchStatus =
        filters.status === "All" ||
      ...
      <div className="bento-filters-grid">
        ...
        <BentoFilterCard
          ...
          options={options.groups.map((g) =>
            g === "All"
              ? "All"
              : `Group ${String.fromCharCode(64 + parseInt(g))}`,
          )}
          onChange={(v) => {
            const val = v.startsWith("Group")
              ? v.split(" ")[1].charCodeAt(0) - 64 + ""
              : "All";
  ...
  const columns = [
        ...
        <Space align="center" size={12}>
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <Tag
                color="blue"
                className="m-0 border-none font-bold text-[11px] px-2 leading-5"
              >
                {record.brand}
              </Tag>
              <Text
    ...
    },
    {
      ...
      render: (record: Shop) => (
        <div
          className={`flex items-start gap-1.5 ${record.status?.toLowerCase() === "closed" ? "opacity-40" : ""}`}
        >
          <EnvironmentOutlined className="text-teal-600 mt-1 text-[13px]" />
          <div className="flex flex-col">
            {/* ✅ 區域與地址文字微調大 */}
            <Text strong className="text-[12px] leading-tight">
              {record.region} · {record.district}
            </Text>
            <Text
              type="secondary"
              className="text-[11px] leading-snug italic line-clamp-1"
            >
              {record.address}
            </Text>
          </div>
      ...
      render: (record: Shop) => {
        const trackingContent = (
          <div className="p-2 w-[260px]" onClick={(e) => e.stopPropagation()}>
            <Text
              strong
              className="text-[10px] uppercase text-slate-400 block mb-2"
            >
              Tracking Log
            </Text>
            <Space direction="vertical" className="w-full" size={8}>
              ...
              >
                ...
                <Option value="No Contact / Wrong No.">
                  No Contact / Wrong No.
                </Option>
              </Select>
              <Input.TextArea
        ...
        return (
          ...
          >
            <div className="flex flex-col min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1">
                <Text type="secondary" className="text-[10px] truncate">
                  <UserOutlined /> {record.contactName || "N/A"}
                </Text>
                {record.callStatus && (
                  <Badge
                    status={
                      record.callStatus === "Called" ? "success" : "warning"
                    }
            ...
            <Popover
              ...
              title={<b className="text-xs">Quick Tracking</b>}
              trigger="click"
              placement="topRight"
            >
              <button
    ...
    },
    {
      ...
      render: (record: Shop) => (
        <div className="flex flex-col gap-0.5 items-left">
          ...
          <Text
            strong
            className="text-[11px] text-indigo-900 whitespace-nowrap"
          >
            {record.scheduledDate
              ? dayjs(record.scheduledDate).format("DD MMM")
              : "TBD"}
          </Text>
          {record.groupId > 0 && (
            <Tag
              className={`m-0 border-none font-black text-[9px] px-1.5 leading-4 tag-group-${record.groupId}`}
            >
              {String.fromCharCode(64 + record.groupId)}{" "}
              {/* 只顯示字母以節省空間 */}
            </Tag>
          )}
        </div>
    ...
    },
    {
      ...
      render: (status: string) => {
        ...
        return (
          <Tag
            color={color}
            className="font-black text-[9px] uppercase border-none px-1.5 rounded-sm m-0 scale-90"
          >
            {status}
          </Tag>
        );
  ...
  return (
    <div className="shop-list-container">
      ...
      <Card
        className="rounded-[16px] border border-slate-200 shadow-md overflow-hidden bg-white mt-6"
