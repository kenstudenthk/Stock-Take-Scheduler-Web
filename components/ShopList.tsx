...
import { Table, Input, Card, Typography, Space, Tag, Empty } from 'antd';
import { SearchOutlined, PlusOutlined, EnvironmentOutlined } from '@ant-design/icons';
import { Shop } from '../types';
import { ShopFormModal } from './ShopFormModal'; // ✅ 確保有引入
...
export const ShopList: React.FC<Props> = ({ shops, graphToken, onRefresh }) => {
  ...
  const [formOpen, setFormOpen] = useState(false);        // 控制彈窗開關
  const [targetShop, setTargetShop] = useState<Shop | null>(null); // 控制是新增(null)還是編輯(object)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: Shop) => (
        <Space direction="vertical" size={0}>
          <Text size="small"><EnvironmentOutlined /> {record.region} - {record.district}</Text>
